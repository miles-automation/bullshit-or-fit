...
| Cloudflare / 636 | Cybersecurity/uptime paragraph concerns retaining customers, not employees | Hiring candidate excluded |
| Twilio / 360 | Past workforce reductions combined with possible future employee growth | Unknown, preserving historical disclosure |
...
- `make check`: passed; **245 backend tests**, including **50 SEC tests**; **21 frontend tests**; Ruff lint/format; mypy across 46 application files; frontend TypeScript.
...
- Behavioral coverage: replay/deduplication, download-budget resumption, separate amendments, malformed arrays/documents, failed-download retry, bounded response/retry behavior, shared request locking/cache, parser table/paragraph boundaries, exact excerpt provenance, risk/negation/mixed context, custom-rule fingerprints, unmatched mappings, and disabled/unavailable/import-failure isolation. Review regressions additionally cover stale/nonfinite lock files, customer-versus-employee retention, historical reductions with future growth, small-budget progress across issuers, corrected URLs, frozen successful provenance, lightweight replay, CLI defaults, discovery coverage warnings, and form/until filters.
...
Claude Code independently reviewed `369c252` and requested changes for a persisted-monotonic-clock stall. This revision clamps and repairs stale/corrupt/nonfinite lock timestamps, honors CLI settings defaults, prevents import failures from killing the worker, avoids replay blob transfers, prioritizes unseen filings ahead of retries, refreshes failed metadata, adds a category index, and expands behavioral coverage. The two additional parent-agent signal findings are fixed under extractor version 3. A fresh independent review of this revision remains required; passing tests and sample inspection are not final acceptance.
